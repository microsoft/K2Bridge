...
namespace K2Bridge.Tests.End2End
{
    ...
    {
        private const string ExpectedInvalidQueryResponse = @"{""responses"":[{""error"":{""root_cause"":[{""type"":""SemanticException"",""reason"":""Semanticerror:'let_data=database(\""build7865\"").kibana_sample_data_flights|where(dayOfWehas\""1\"")and(timestamp>=unixtime_milliseconds_todatetime(1517954400000)andtimestamp<=unixtime_milliseconds_todatetime(1518127199999));(_data|summarizecount()bytimestamp=startofmonth(timestamp)|orderbytimestampasc|asaggs);(_data|orderbytimestampdesc|limit500|ashits)'hasthefollowingsemanticerror:'where'operator:Failedtoresolvescalarexpressionnamed'dayOfWe'."",""index_uuid"":""kibana_sample_data_flights"",""index"":""kibana_sample_data_flights""}],""type"":""QueryException"",""reason"":""Failedexecutingazuredataexplorerquery"",""phase"":""query""},""status"":500}]}";
